...
namespace WebTransport
{
    public partial class RateMasterRetail : Pagebase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                lbltxt.Text = "Rate Master Retailer";
            }
            else if (HidInvoiceTyp.Value == "TBB")
            {
                lbltxt.Text = "Rate Master Retailer [TBB]";
            }            

            ddlDateRange.Focus();
            RateMasterRetailerDAL objuser = new RateMasterRetailerDAL();
            tblUserPref objpref = objuser.selectUserPref();
            //if (Convert.ToBoolean(objpref.Cont_Rate) == true)
            //{
            //    lblContSize.Visible = true;
            //    drpConSize.Visible = true; 
            //}
            //else
            //{
            //    lblContSize.Visible = false;
            //    drpConSize.Visible = false; 
            //}
        }
        #endregion
        ...
        {
            RateMasterRetailerDAL objRateMst = new RateMasterRetailerDAL();
        ...
        protected void lnkbtnSave_OnClick(object sender, EventArgs e)
        {
            ...
            tblRateMastRetailer objRGH = new tblRateMastRetailer();
            ...
            List<tblRateMastRetailer> RgDlst = new List<tblRateMastRetailer>();
            ...
            else
            {
                ...
                {
                    isinsert = obj.Insert(DtTemp, Convert.ToInt32(ddlItemName.SelectedValue), Convert.ToString(HidInvoiceTyp.Value) == "" ? "IR" : Convert.ToString(HidInvoiceTyp.Value), Convert.ToInt32(drpBaseCity.SelectedValue), Convert.ToInt32(drpBaseCity.SelectedValue), empIdno,Convert.ToString(ddlTranType.SelectedItem.Text),Convert.ToInt64(ddlTranType.SelectedValue));
                    this.Clear();
        ...
        protected void lnkbtnSubmit_OnClick(object sender, EventArgs e)
        {
            ...
            if (IsWeight == false)
            {
                ...
                {
                    if (((string.IsNullOrEmpty(Convert.ToString(txtItemRate.Text.Trim())) ? 0 : Convert.ToDouble(txtItemRate.Text.Trim())) <= 0) && ((string.IsNullOrEmpty(Convert.ToString(txtItemWighRate.Text.Trim())) ? 0 : Convert.ToDouble(txtItemWighRate.Text.Trim())) <= 0))
                    {
                        this.ShowMessageErr(" Item Rate or Weight must be greater than 0!"); txtItemRate.Focus(); txtItemRate.SelectText();
                        return;
                    }
                }
                else
            ...
            //RateMasterRetailerDAL objDal = new RateMasterRetailerDAL();
                ...
                if (DtTemp != null)
                {
                    foreach (DataRow dtrow in DtTemp.Rows)
                    {
                        if (Convert.ToString(dtrow["id"]) == Convert.ToString(Hidrowid.Value))
                        {
                            ...
                            dtrow["TranType_Idno"] = ddlTranType.SelectedValue;
                            dtrow["Tran_Type"] = ddlTranType.SelectedItem.Text;
                            lnkbtnSubmit.Focus();
                        }
                    }
                }
        ...
        private void BindGridDB()
        {
            ...
            dt1 = objRateMst.SelectDBData(Convert.ToInt64(ddlItemName.SelectedValue), Convert.ToInt32(drpBaseCity.SelectedValue), Convert.ToString(HidInvoiceTyp.Value), Convert.ToInt32(ddlCity.SelectedValue) == 0 ? 0 : Convert.ToInt32(ddlCity.SelectedValue), Convert.ToInt32(ddlTranType.SelectedValue) == 0 ? 0 : Convert.ToInt32(ddlTranType.SelectedValue), ApplicationFunction.ConnectionString());
            ...
            if (dt1 != null && dt1.Rows.Count > 0)
            {
                for (int i = 0; i < dt1.Rows.Count; i++)
                {
                    ...
                    Dr[25] = Convert.IsDBNull(dt1.Rows[i]["Tran_Type"]) ? "0" : Convert.ToString(dt1.Rows[i]["Tran_Type"]).ToString();
                    Dr[26] = Convert.IsDBNull(dt1.Rows[i]["TranType_Idno"]) ? "0" : Convert.ToString(dt1.Rows[i]["TranType_Idno"]).ToString();
        ...
        protected void ddlItemName_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlItemName.SelectedIndex > 0)
            {
                ...
                //RateMasterRetailerDAL objclsOpeningStock = new RateMasterRetailerDAL();
                //DtTemp = null;
                //ViewState["dt"] = null;
