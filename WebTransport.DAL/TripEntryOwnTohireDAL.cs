...
{
    public class TripEntryOwnTohireDAL
    {
        ...
        public Int32 GetTripNo(Int32 YearIdno, Int32 FromCity, string Constring)
        {
            using (TransportMandiEntities db = new TransportMandiEntities(MultipleDBDAL.strDynamicConString()))
            {
                ...
                Int32 MaxNo = 0;
                MaxNo = Convert.ToInt32(SqlHelper.ExecuteScalar(Constring, CommandType.StoredProcedure, "spTripSheetOwnToHire", objSqlParameter));
                return MaxNo;
            }
        }
        ...
        public IList search(Int32 yearid, string TripNo, DateTime? dtfrom, DateTime? dtto, int FromCity, Int32 TruckNo, Int64 UserIdno)
        {
            using (TransportMandiEntities db = new TransportMandiEntities(MultipleDBDAL.strDynamicConString()))
            {
                var lst = (from CH in db.tblTripOwnToHireHeads
                ...
                if (TripNo != "")
                {
                    lst = lst.Where(l => l.TripOTH_No == TripNo).ToList();
                }
                if (dtto != null)
                {
                    lst = lst.Where(l => Convert.ToDateTime(l.TripOTH_Date).Date <= Convert.ToDateTime(dtto).Date).ToList();
                }
                if (dtfrom != null)
                {
                    lst = lst.Where(l => Convert.ToDateTime(l.TripOTH_Date).Date >= Convert.ToDateTime(dtfrom).Date).ToList();
                }
                if (FromCity > 0)
                {
        ...
        public tblTripOwnToHireHead selectHead(Int64 HeadId)
        {
            using (TransportMandiEntities db = new TransportMandiEntities(MultipleDBDAL.strDynamicConString()))
            {
                return db.tblTripOwnToHireHeads.Where(h => h.TripOTH_Idno == HeadId).FirstOrDefault();
            }
        }
        ...
        public DataSet selectDetl(string con, Int64 iYearId, Int64 HeadId)
        {
            ...
            DataSet objDsTemp = SqlHelper.ExecuteDataset(con, CommandType.StoredProcedure, "spTripSheetOwnToHire", objSqlPara);
        ...
        public IList BindInvoice(Int64[] Ids)
        {
            using (TransportMandiEntities db = new TransportMandiEntities(MultipleDBDAL.strDynamicConString()))
            {
                return (from obj in db.TblHireInvHeads
                        where Ids.Contains(obj.Hire_Idno)
                        select new
                        {
                            InvoNo = obj.Hire_InvNo,
                            InvoIdno = obj.Hire_Idno,
                            DriverIdno = 0,
                            Date = obj.Hire_Date,
                            Gross = (obj.Gross_Amnt),
                            Advance = (obj.Adv_Amnt) ,
                            Total = (obj.Net_Amnt)
                        }).GroupBy(x => x.InvoIdno).Select(x => x.FirstOrDefault()).ToList();
            }
        ...
        public IList SearchGrD(int SearchType, DateTime? DocFromDate, DateTime? DocToDate, Int32 DocNo, Int64 LocIdno, Int64 TruckIdno)
        {
            using (TransportMandiEntities db = new TransportMandiEntities(MultipleDBDAL.strDynamicConString()))
            {
                if (SearchType == 0)
                {
                    var Result = (from obj in db.TblHireInvHeads
                                  join Dt in db.TblHireInvDetls on obj.Hire_Idno equals Dt.Hire_Idno
                                  where obj.Loc_Idno == LocIdno && Dt.Truck_Idno == TruckIdno //&& (obj.Trip_Billed == null || obj.Trip_Billed == false) && Dt.Delivered == true
                                  select new
                                  {
                                      DocNo = obj.Hire_InvNo,
                                      DocDate = obj.Hire_Date,
                                      DocAmnt = obj.Gross_Amnt,
                                      DocType = "Invoice Hire",
                                      DocIdno = obj.Hire_Idno,
                                      Driver_Idno=0
                                  }).Distinct().ToList();
                    if (DocToDate != null)
                    {
                    ...
                    if (Result.Count > 0)
                    {
                        if (DocNo != 0)
                        {
                            Result = Result.Where(r => r.DocNo == Convert.ToInt32(DocNo)).ToList();
                        }
                    }
                    return Result;
                }
